...
export default function Industries() {
  return (
    <section id="industries" className="py-16 md:py-24 lg:py-32 bg-[#0F1A2E] relative overflow-hidden">
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[700px] h-[400px] bg-blue-700/5 rounded-full blur-[110px]" />
      </div>
      ...
      <div className="max-w-6xl mx-auto px-5 md:px-6 relative">
        ...
        >
          <p className="text-orange-400 text-sm font-semibold tracking-widest uppercase mb-4">Industries</p>
          ...
          {industries.map((ind, i) => {
            ...
            return (
              <motion.div
                ...
                className="group bg-[#0A0F1E]/60 border border-[#1E3A8A]/30 rounded-2xl p-5 md:p-8 flex flex-col items-center gap-3 md:gap-4 text-center hover:border-[#3B82F6]/50 hover:bg-[#0A0F1E] hover:shadow-[0_0_50px_rgba(59,130,246,0.1)] transition-all duration-300 cursor-pointer min-h-[120px] md:min-h-0"
              >
                <motion.div
                  whileHover={{ scale: 1.18, rotate: [0, -4, 4, 0] }}
                  transition={{ type: "spring", stiffness: 300, damping: 15 }}
                  className="w-14 h-14 rounded-2xl bg-orange-500/10 border border-orange-500/20 flex items-center justify-center text-orange-400 group-hover:text-orange-300 group-hover:border-orange-400/40 transition-colors duration-300"
                >
                  {ind.icon}
                </motion.div>
